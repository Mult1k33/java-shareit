...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(Long ownerId, ItemNewDto itemNewDto) {
        ...
        if (itemNewDto.getRequestId() != null) {
            request = itemRequestRepository.findById(itemNewDto.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запрос с ID" + itemNewDto.getRequestId() + " не найден"));
        }
        Item item = ItemMapper.mapToItem(itemNewDto, owner, request);
    ...
    public Collection<ItemDto> findItemsByRequestIds(List<Long> requestIds) {
        if (requestIds == null || requestIds.isEmpty()) {
            return Collections.emptyList();
        }

        Collection<Item> items = itemRepository.findByRequestIds(requestIds);
        return items.stream()
                .map(ItemMapper::mapToDto)
                .collect(Collectors.toList());
    }
